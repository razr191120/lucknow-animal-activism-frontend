...
export default function RescueDetail() {
  ...
  if (error || !id) {
    return (
      <div className="max-w-7xl mx-auto px-4 py-8 text-red-700">
        {error || 'Invalid link.'}{' '}
        <Link to={LA.rescues} className="underline">
          Back
        </Link>
      </div>
    );
  ...
  return (
    <article className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-6">
      ...
      <header>
        <span className="rounded bg-laap-100 px-2 py-1 text-xs font-bold uppercase text-laap-800">
          {row.urgency}
        </span>
        <h1 className="mt-2 text-2xl font-black text-laap-950">{row.title}</h1>
      ...
      <section className="rounded-xl border border-laap-200 bg-white p-5 shadow-sm">
        <h2 className="font-bold text-laap-900">Location</h2>
        <p className="mt-2 whitespace-pre-wrap text-earth-800">{row.location_address}</p>
        <h2 className="mt-4 font-bold text-laap-900">Situation</h2>
        <p className="mt-2 whitespace-pre-wrap text-earth-800">
          {row.description || '—'}
        </p>
        {row.animal_condition && (
          <p className="mt-3 text-sm text-earth-700">
            <span className="font-semibold">Condition:</span> {row.animal_condition}
          </p>
        )}
        {row.contact_phone && (
          <p className="mt-2 text-sm text-earth-700">
